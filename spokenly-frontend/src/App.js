...
function generateUniqueSessionID() {
  // This will generate a random number between 100000 and 999999
  return Math.floor(100000 + Math.random() * 900000);
}

const loadSharedDataFromAPI = async (sessionId) => {
    try {
      const response = await fetch(`${apiUrl}/transcripts_history/${sessionId}`, {
        method: 'GET',
        headers: {
      ...
      console.error('Error loadDataFromAPI', error);
    }
...
const App = () => {
  ...
  // --- Debounced API call for chunk size change ---
    useEffect(() => {
        // This effect runs whenever 'chunkSize' changes.
        ...
        const handler = setTimeout(() => {
            // 3. This code runs after the timer finishes (3 seconds).
            // In a real app, you would make your fetch() API call here.
            // For this demo, we'll just show a status message.
            updateSettings({ "chunk_size" : chunkSize })
        }, 1000); // Wait for 3 seconds of inactivity

        // 2. Cleanup function: This is the key to debouncing.
        // If 'chunkSize' changes again, React will run this cleanup function before running the effect again.
        // This clears the previously set timer, effectively resetting the 3-second wait period.
        return () => {
  ...
  }, [transcriptHistory, currentUtterance, autoScroll]);
  ...
  const stopRecording = useCallback(() => {
    ...
    try {
      ...
      console.warn("Error while stopping recording:", err);
    }
  ...
  const loadSharedData = useCallback(async() => {
    ...
    if (shareData) {
      try {
        const decodedData = await loadSharedDataFromAPI(shareData)
  ...
  const handleChunkChange = (amount) => {
    setChunkSize(prev => Math.max(5, Math.min(120, prev + amount))); // Ultra-low latency range
  }
          ...
          <section className="controls-section">
            <div className="control-group">
              ...
              <div className="chunk-controls">
                ...
                <button className="control-btn" onClick={() => handleChunkChange(5)} disabled={chunkSize >= 120}>+</button>
              </div>
            </div>
            <div className="control-group">
              <label>Speed</label>
          ...
          <div className="modal-overlay" onClick={() => setShowShareModal(false)}>
            <div className="modal-content" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="share-url">
                <input type="text" value={shareUrl} readOnly style={{ width: '100vw' }}/>
